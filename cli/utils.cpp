...
std::regex regex_literal_dec = std::regex("[dD]'([0-9]+)'|([0-9A-Fa-f]+)");
...
int to_literal(std::string const &literal_str) {
  int literal = 0;
  std::smatch sm;
  if (std::regex_match(literal_str, sm, utils::regex_literal_dec)) {
    std::stringstream ss;
    ss << utils::get_first_not_empty_submatch(sm);
    ss >> literal;
  } else if (std::regex_match(literal_str, sm, utils::regex_literal_hex)) {
    std::stringstream ss;
